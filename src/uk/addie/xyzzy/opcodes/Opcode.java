...
@SuppressWarnings({ "unused" }) public enum Opcode {
  ...
  GET_CURSOR(3, 0x10) {
    @Override public void invoke(final ShortStack arguments) {
      ...
      final Point cursor = Memory.current().zwin.get(Memory.current().currentScreen)
          .cursorPosition();
  ...
  RESTORE(4, 0x1) {
    @Override public void invoke(final ShortStack arguments) {
      ...
      if (!loaded.storyPath.equals(Memory.current().storyPath)) {
        Log.i("Xyzzy", "Not a save from this story. " + loaded.storyPath + " v "
            + Memory.current().storyPath);
        readDestinationAndStoreResult(0);
      } else {
  ...
  SAVE(4, 0x0) {
    @Override public void invoke(final ShortStack arguments) {
      try {
        ...
        final FileOutputStream save = MainActivity.activity.openFileOutput(saveGameName,
  ...
  SAVE_UNDO(4, 0x9) {
    @Override public void invoke(final ShortStack arguments) {
      ...
      Log.i("Xyzzy",
          "Made undo file:" + Memory.undo().length + " bytes, "
  ...
  SCAN_TABLE(3, 0x17) {
    @Override public void invoke(final ShortStack arguments) {
      ...
      for (int i = 0; i < len; i += skip) {
        final int wordAtI = isWord ? Memory.current().buffer.getShort(table + i * skip) & 0xffff
  ...
  SHOW_STATUS(0, 0xc) {
    @Override public void invoke(final ShortStack arguments) {
      if (Header.VERSION.value() > 3) {
        ...
        final boolean scoreGame = Header.VERSION.value() < 2 || !Bit.bit1(flags);
        final StringBuilder sb = new StringBuilder();
        final int firstGlobal = readValue(16);
        final ZObject zo = ZObject.count(firstGlobal);
        ...
        if (scoreGame) {
          ...
          if ((Boolean) Preferences.TWENTYFOURHOUR.getValue(MainActivity.activity)) {
            ...
            sb.append(String.format("Time: %d:%02d %s", displayHours, minutes, hours < 12 ? "am"
                : "pm"));
          }
        }
  ...
  @Override public String toString() {
    return "(" + operands + "," + Integer.toHexString(hex) + ") "
        + super.toString().toLowerCase(Locale.UK);
  }
  ...
  static void storeValue(final int destination, final int value) {
